...
export default function CartItem({ item, isLast }: CartItemProps) {
  ...
  const stock = product ? (product as any).stock ?? 0 : 0;
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between">
            <div className="flex-1 min-w-0 pr-4">
              ...
              {isOutOfStock ? (
                <p className="text-xs text-red-600 font-bold mt-1.5 bg-red-50 px-2 py-0.5 rounded border border-red-100 inline-block">Out of Stock</p>
              ) : isExceedingStock ? (
                <p className="text-xs text-orange-600 font-bold mt-1.5 bg-orange-50 px-2 py-0.5 rounded border border-orange-100 inline-block">Only {stock} available</p>
              ) : null}
            </div>
